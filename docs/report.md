...
The solution is currently hosted in [image-gallery.goncalo2k.pt](https://image-gallery.goncalo2k.pt) and its Github repo is under [goncalo2k/image-gallery](https://github.com/goncalo2k/image-gallery).
...
- **Secure touchpoints** – Cloudflare Access JWT assertions, CORS allow-lists, and signed-token verification against the team's JWKS endpoint ensure that only trusted systems can mutate the gallery, while public endpoints (`/health`, `/docs`, `/openapi.json`) remain open for observability.
...
Send `GET https://image-gallery.goncalo2k.pt/images/audit?offset=0&limit=20` with a valid `cf-access-jwt-assertion` header issued by Cloudflare Access to retrieve recent uploads plus aggregate stats (`totalImages`, `lastUpdated`). Use this endpoint to populate dashboards or monitor image insertion or deletion. Note that this endpoint is also paginated.
...
- **Cloudflare Access** – Protects `/images/audit` (and any future admin route) with Access-issued JWTs enforced by `auth.middleware.ts`.
...
1. **Authenticated admin routes** – `auth.middleware.ts` inspects the `cf-access-jwt-assertion` header, validates the token with Cloudflare Access JWKS keys, and scopes protection via `AUTH_ROUTES` so sensitive endpoints (e.g., `/images/audit`) require a valid Access-issued JWT.
2. **Least-privilege bindings** – `wrangler.jsonc` binds only the required resources (R2, D1, Images, AI) plus explicit env vars. Secrets never live in code, and `.dev.vars.example` documents the Access audience, team domain, and environment placeholders needed so local dev mirrors production without credential reuse.
